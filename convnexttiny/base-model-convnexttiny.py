...
RUN_LOG_PATH = f"convnext_tiny_training_log_{DATA_SUBSET.lower()}.jsonl"

GRADCAM_OUTPUT_DIR = f"gradcam_CONVNEXT_TINY_{DATA_SUBSET.lower()}_infected_predictions"
MODEL_SAVE_PATH = f"convnext_tiny_{DATA_SUBSET.lower()}_salmonscan.pth"
...
# این دیتاست برای استخراج مسیر اصلی تصاویر در Grad-CAM هم استفاده می‌شود.
val_base_dataset = datasets.ImageFolder(
    root=VAL_DATA_DIR,
    transform=val_transform
)

# برای سازگاری با تابع Grad-CAM در common_utils
# چون آن تابع معمولاً انتظار دارد test_dataset.indices وجود داشته باشد.
val_dataset = Subset(
    val_base_dataset,
...
def main():
    if MODE == "train":
        print("\n--- Running Mode: TRAIN (ConvNeXt-Tiny) ---")
    ...
    print("\n===== Validation/Test Results: ConvNeXt-Tiny =====")
    ...
    plt.title(f"Confusion Matrix - {DATA_SUBSET} - ConvNeXt-Tiny")
    plt.tight_layout()
    plt.show()
